...
#### Методы
  ...
  - `handleResponse` - отвечает за обработку ответов сервера;
...
#### Методы
  ...
  - `changeBasketCounter` - счётчик товаров в корзине;
  - `getProductIds` - получает id товара;
  - `makeOrder` - осуществляет проверку товаров;
...
#### Методы
  - `getProductList` - получает список всех доступных продуктов с сервера;
  - `getProductItem` - получает данные продукта по его идентификатору;
  - `orderProducts` - отправляет информацию о заказе на сервер;
...
#### Методы
  - `set id(value: string)` - сеттер id товара;
  - `get id(): string` - геттер id товара;
...
#### Методы
  ...
  - `get inBasket(): boolean` - геттер наличия товара в корзине;
  - `toggleButtonCondition` - изменяет текст на кнопке товара;
  - `render` - отображение товара в модальном окне; 
...
#### Методы
  - `set content` - сеттер контента модального окна;
...
#### Поля класса
- `_payment:  HTMLButtonElement[]` - кнопки для выбора способа оплаты;

#### Методы
- `set address(value: string)` - сеттер адреса доставки;
- `paymentSelection` - метод выбора способа оплаты;
- `selectButton` - выбор кнопки способа оплаты;
...
#### Поля класса
  ...
  - `_description: HTMLElement` - информация о списанной сумме;  

#### Методы
  - `set total(value: number)` - сеттер списанной суммы;
  - `set id(value: string)` - сеттер идентификатора;

## Список событий возникающих в приложении

 - `catalog:changed` - обновление каталога товаров при загрузке страницы;
 - `card:open` - создаётся и отображается карточка товара в модальном окне;
 - `product:add` - добавление товара в корзину;
 - `product:remove` - удаление товара из корзины;
 - `counter:changed` - обновляется общее количество товаров в корзине и список товаров в корзине;
 - `basket:open` - открытие корзины с товарами;
 - `order:open` - открытие окна ввода способа оплаты и адреса доставки;
 - `orderErrors:changed` - обработка ошибок в полях ввода формы;
 - `paymentForm:changed` - обрабатывает изменения в полях ввода формы для выбора способа оплаты и адреса доставки;
 - `paymentForm:submit` - открывает модальное окно для ввода контактной информации;
 - `contactForm:changed` - обрабатывает изменения в полях ввода формы контактов;
 - `contactForm:submit` - осуществляется заказ товаров, корзина очищается, показывается сообщение об успешном завершении;
 - `modal:open` - при открытии модального окна блокируется прокрутка страницы;
 - `modal:close` - при закрытии модального окна разблокируется прокрутка страницы;